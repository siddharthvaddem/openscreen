...
function Get-CustomCursorType($bitmap, $hotspotX, $hotspotY) {
    ...
    # Arrow cursor: hotspot is at the very top-left corner of the *opaque* area.
    #
    # We measure the hotspot's distance from the opaque bounding box corner
    # ($left, $top) rather than its absolute position in the bitmap.  This is
    # scale-independent: Chrome's private arrow cursor keeps its tip at the
    # top-left of the opaque area regardless of whether the bitmap is rendered
    # at 1× (hotspot ≈ 3 px from the bitmap edge) or 2× HiDPI (hotspot ≈ 8 px
    # from the bitmap edge but still ≈ 0 px from the opaque area's corner).
    #
    # An extra guard — significant rightward extent from the hotspot — distinguishes
    # the diagonal arrow body from a pointer finger that extends mainly downward.
    $hotspotNearOpaqueLeft = ($hotspotX - $left) -le [Math]::Max(2, $bitmap.Width * 0.10)
    $hotspotNearOpaqueTop  = ($hotspotY - $top)  -le [Math]::Max(2, $bitmap.Height * 0.10)
    $arrowRightExtent = [double]($right - $hotspotX) / $bitmap.Width
    if ($hotspotNearOpaqueLeft -and $hotspotNearOpaqueTop -and
        $arrowRightExtent -gt 0.30 -and
        $opaqueWidth -gt ($bitmap.Width * 0.2) -and $opaqueHeight -gt ($bitmap.Height * 0.3)) {
        return 'arrow'
    }

    # Pointer / hand cursor: hotspot is at the fingertip — near the top of the
    # opaque area but NOT at the far-left corner (arrow was already handled above).
    # We again measure distance from the opaque bounding box rather than using
    # an absolute percentage of the full bitmap, so this works at any DPI scale.
    $hotspotNearOpaqueTop2 = ($hotspotY - $top) -le [Math]::Max(2, $bitmap.Height * 0.15)
    if ($hotspotNearOpaqueTop2 -and
        $hotspotX -gt ($bitmap.Width * 0.05) -and $hotspotX -lt ($bitmap.Width * 0.90) -and
