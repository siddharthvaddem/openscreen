// Shared project schema — validation, normalization, and creation of .openscreen project files.
// Pure TypeScript, no browser dependencies.
// Original source: src/components/video-editor/projectPersistence.ts

import { ASPECT_RATIOS, type AspectRatio, isPortraitAspectRatio } from "./aspect-ratios";
import type { ExportFormat, ExportQuality, GifFrameRate, GifSizePreset } from "./export-types";
import type { ProjectMedia } from "./recording-session";
import { normalizeProjectMedia } from "./recording-session";
import {
		...
		? editor.annotationRegions
				...
				.map((region, index) => {
					...
					const blurShape =
						...
						(VALID_BLUR_SHAPES as ReadonlySet<string>).has(region.blurData.shape)
